...
function getPatchTargetSignature(twPatcher) {
  ...
  return `${_nullishCoalesce(_optionalChain([packageInfo, 'optionalAccess', _5 => _5.rootPath]), () => ( "missing"))}:${_nullishCoalesce(_optionalChain([packageInfo, 'optionalAccess', _6 => _6.version]), () => ( "unknown"))}:${_nullishCoalesce(twPatcher.majorVersion, () => ( "unknown"))}`;
}
...
function logTailwindcssTarget(kind, patcher, baseDir) {
  const packageInfo = _optionalChain([patcher, 'optionalAccess', _25 => _25.packageInfo]);
  const label = kind === "cli" ? "weapp-tw patch" : "tailwindcss-patcher";
  if (!_optionalChain([packageInfo, 'optionalAccess', _26 => _26.rootPath])) {
    _logger.logger.warn(
...
function normalizeOptionsValue(rawValue, stack = /* @__PURE__ */ new WeakSet()) {
  ...
  if (ArrayBuffer.isView(rawValue)) {
    ...
    return encodeTaggedValue(_nullishCoalesce(_optionalChain([view, 'access', _28 => _28.constructor, 'optionalAccess', _29 => _29.name]), () => ( "ArrayBufferView")), buffer.toString("base64"));
  }
...
function createNameMatcher(list, { exact = false } = {}) {
  ...
  return (value) => {
    if (exact && _optionalChain([exactStrings, 'optionalAccess', _30 => _30.has, 'call', _31 => _31(value)])) {
      return true;
    }
    if (_optionalChain([fuzzyMatcher, 'optionalCall', _32 => _32(value)])) {
      return true;
    }
    if (!hasRegex) {
      return false;
...
function processUpdatedSource(rawSource, options, analysis) {
  ...
  for (const path5 of targetPaths) {
    ...
    const token = replaceHandleValue(
      ...
      {
        ...options,
        needEscaped: path5.isStringLiteral() ? _nullishCoalesce(options.needEscaped, () => ( true)) : false
      }
    );
    if (token) {
