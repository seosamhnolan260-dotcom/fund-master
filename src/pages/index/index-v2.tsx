...
// --- 内部组件：AI 研报核心摘要 ---
const AnalysisReport: React.FC<{ data: any }> = ({ data }) => {
  ...
  const rating = (() => {
    ...
    if (pePercentile < 30 && return1Year > 10) return { text: "强烈推荐", color: "#34C759" };
    if (pePercentile < 50 && return1Year > 5) return { text: "推荐", color: "#007AFF" };
    if (pePercentile < 70) return { text: "中性", color: "#FF9500" };
    return { text: "谨慎", color: "#FF3B30" };
  })();
  ...
  const valuationComment = (() => {
    ...
    if (pePercentile < 20) return "当前估值处于历史低位，具备较好配置价值，建议重点关注。";
    if (pePercentile < 50) return "当前估值处于合理区间，适合定投布局。";
    if (pePercentile < 80) return "当前估值处于历史中高位，建议谨慎配置。";
    return "当前估值处于历史高位，建议等待回调机会。";
  })();
  
  // 计算重仓股集中度
  const topHoldingsConcentration = (data?.top_stocks || [])
    .reduce((sum: number, stock: any) => sum + (typeof stock.ratio === 'string' ? parseFloat(stock.ratio) : (stock.ratio || 0) * 100), 0)
    .toFixed(2);
  ...
  return (
    <View className="ai-report">
      ...
      <View style={{ marginBottom: '24px' }}>
        <Text style={{ fontSize: '18px', fontWeight: '700', color: '#003371', marginBottom: '16px' }}>核心摘要</Text>
        ...
        <View style={{ 
          ...
          <Text style={{ fontSize: '14px', color: 'rgba(255,255,255,0.9)', marginBottom: '8px' }}>投资评级</Text>
          <Text style={{ fontSize: '28px', fontWeight: '700', color: '#FFFFFF' }}>{rating.text}</Text>
        </View>
        
        {/* 核心观点 */}
        <View style={{ marginBottom: '16px' }}>
          <Text style={{ fontSize: '14px', color: '#6E6E73', marginBottom: '8px' }}>核心观点</Text>
          <Text style={{ fontSize: '15px', lineHeight: '1.7', color: '#1D1D1F' }}>
            • {data?.fund_name || "该基金"}在{data?.fund_type || "同类基金"}中表现
            {parseFloat(data?.return_1year || "0") > 10 ? "优异" : "良好"}
            ，近一年收益{data?.return_1year || "数据暂缺"}。{'\n'}
            • {valuationComment}{'\n'}
            • 前十大重仓股集中度{topHoldingsConcentration}%，
            {parseFloat(topHoldingsConcentration) > 50 ? "集中度较高" : "配置分散"}。
          </Text>
        </View>
        ...
        <View style={{ 
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: '12px', color: '#86868B', marginBottom: '4px' }}>PE 分位</Text>
            <Text style={{ fontSize: '16px', fontWeight: '600', color: '#1D1D1F' }}>
              {data?.pe_percentile ? `${data.pe_percentile}%` : "数据暂缺"}
            </Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: '12px', color: '#86868B', marginBottom: '4px' }}>集中度</Text>
            <Text style={{ fontSize: '16px', fontWeight: '600', color: '#1D1D1F' }}>
              {topHoldingsConcentration}%
            </Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: '12px', color: '#86868B', marginBottom: '4px' }}>规模</Text>
            <Text style={{ fontSize: '16px', fontWeight: '600', color: '#1D1D1F' }}>
              {data?.fund_scale || "数据暂缺"}
            </Text>
          </View>
        </View>
      </View>
      ...
      <View className="report-footer">
        <Text>数据来源：盈米 MCP | 生成时间：{today}</Text>
        <Text style={{ fontSize: '11px', color: '#86868B', marginTop: '8px' }}>
          点击"下载完整研报"获取 11 章专业版报告
        </Text>
      </View>
    </View>
  );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.statusCode === 200 && res.data?.success) {
        ...
        console.log("单位净值:", fundData.net_value);
        console.log("近 1 年收益:", fundData.return_1year);
        console.log("PE 分位:", fundData.pe_percentile);
        console.log("重仓股:", fundData.top_stocks);
        console.log("==========================");
